...
// auth.php doesn't always guarantee a started session for this page
    ...
    if (!$email || !$password) {
        ...
        if (!$user) {
            ...
            if (isset($demoUsers[$email]) && $password === 'Password@123') {
                $u = $demoUsers[$email];
                // Set session user using the same shape expected by authUser()
                $_SESSION['user'] = [
                    'id' => $u['id'],
                    'name' => $u['name'],
                    'email' => $email,
            ...
            // Use existing auth helper to set session user
            $_SESSION['user'] = [
                'id' => $user['id'],
                'name' => $user['name'],
                'email' => $user['email'],
...
// Ensure session is available for authUser() and setting $_SESSION['user']
if (session_status() === PHP_SESSION_NONE) {
    session_start();
}
